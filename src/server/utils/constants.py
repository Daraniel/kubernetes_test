...
if 'ACCESS_TOKEN_EXPIRE_MINUTES' not in os.environ:
    logger.critical('ACCESS_TOKEN_EXPIRE_MINUTES environment variable not set, using the its default value.')
    os.environ['ACCESS_TOKEN_EXPIRE_MINUTES'] = '30'
try:
    ACCESS_TOKEN_EXPIRE_MINUTES = int(os.getenv('ACCESS_TOKEN_EXPIRE_MINUTES'))
except ValueError:
    # Handle the exception
    raise Exception("ACCESS_TOKEN_EXPIRE_MINUTES environment variable must be an integer.")
