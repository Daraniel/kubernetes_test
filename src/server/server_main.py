...
def database_error_handler(_: Request, exc: DatabaseError):
    ...
    return JSONResponse(status_code=422, content={"error": exc.__class__.__name__, "message": exc.args[0]})
...
async def add_user(
        current_user: Annotated[User, Depends(get_current_active_user)],
        username: str, full_name: str, email: str, password: str, disabled: bool = False,
        is_admin: bool = False,
    ...
    db.create_user(username, full_name, email, get_password_hash(password), disabled, is_admin)
...
async def get_all_users(
    ...
    if not current_user.is_admin:
        raise HTTPException(
            status_code=401,
            detail="Only admin users can see other users.")
    return db.get_all_users()


@app.get("/admin/users/delete_user/")
async def delete_user(
        current_user: Annotated[User, Depends(get_current_active_user)], username: str,
        db: DatabaseManager = Depends(get_db)):
    if not current_user.is_admin:
        raise HTTPException(
            status_code=401,
            detail="Only admin users can remove users.")
...
async def deactivate_user(
    ...
    if not current_user.is_admin:
        raise HTTPException(
            status_code=401,
            detail="Only admin users can disable users.")
...
async def activate_user(
    ...
    if not current_user.is_admin:
        raise HTTPException(
            status_code=401,
            detail="Only admin users can enable users.")
...
async def promote_user(
    ...
    if not current_user.is_admin:
        raise HTTPException(
            status_code=401,
            detail="Only admin users can make users admin.")
...
async def add_own_item(
        current_user: Annotated[User, Depends(get_current_active_user)],
        item_name: str, description: str = None, price: float = None,
        db: DatabaseManager = Depends(get_db)
):
    db.create_item(name=item_name, owner=current_user.id, description=description, price=price)
    return 200
...
async def add_item(
    ...
    if not current_user.is_admin:
        raise HTTPException(
            status_code=401,
            detail="Only admin users can create item for another user.")
...
async def delete_own_item(
        current_user: Annotated[User, Depends(get_current_active_user)],
        item_name: str, db: DatabaseManager = Depends(get_db)):
    ...
    if item.owner == current_user.id:
        ...
        raise HTTPException(
            status_code=401,
            detail="Item doesn't belong to the user.")
    return 200


@app.get("/admin/items/delete_item/")
async def delete_item(
        current_user: Annotated[User, Depends(get_current_active_user)], item_name: str,
        db: DatabaseManager = Depends(get_db)):
    if not current_user.is_admin:
        raise HTTPException(
            status_code=200,
            detail="Only admin users can delete other user's items.")
...
async def get_item_by_name(
    ...
    if not current_user.is_admin:
        raise HTTPException(
            status_code=401,
            detail="Only admin users can see other user's items.")
...
async def get_own_items(
    ...
    return [InventoryItem(**item.__dict__) for item in db.get_items_by_owner(current_user.id)]


@app.get("/admin/items/get_items/", response_model=List[InventoryItem])
async def get_items(
        current_user: Annotated[User, Depends(get_current_active_user)], user_name: str,
        db: DatabaseManager = Depends(get_db)
):
    if not current_user.is_admin:
        raise HTTPException(
            status_code=401,
            detail="Only admin users get other user's items.")
    user = db.get_user_by_username(user_name)
    if user is None:
        raise HTTPException(
            status_code=403,
